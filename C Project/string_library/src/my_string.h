...
동적 문자열 라이브러리를 만드는 이유: 문자열 관련 함수들을 직접 구현해보면서 C언어의 문자열 처리 방식을 더 깊이 이해하고, 포인터와 메모리 관리를 연습하기 위해서이다.
이 라이브러리는 C언어의 표준 문자열 함수들과 유사한 기능을 제공하지만, 직접 구현함으로써 내부 동작 방식을 학습할 수 있다.
*/
