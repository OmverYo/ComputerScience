...
int main()
{
    ...
    // 내 질문:
    // 지금 배우는 malloc/calloc이 결국 배열을 만드는건데 왜 굳이 이걸 써야됨?
    // 배열은 고정 크기인데, malloc/calloc은 동적 크기 할당이 가능해서 유연함
    // 그리고 포인터 연산을 통해 배열처럼 접근 가능
    // 일반 배열은 스택 메모리에 할당되지만, malloc/calloc은 힙 메모리에 할당됨

    // 스택과 힙 차이:
    // 스택: 함수 호출 시 자동으로 할당/해제, 크기 제한적
    // 힙: 프로그래머가 직접 할당/해제, 큰 메모리 사용 가능
